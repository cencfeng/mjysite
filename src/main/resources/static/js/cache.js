...
layui.use(['form','jquery',"layer"],function() {
    ...
    //公告层
    function showNotice(){
        $.post("/system/gettip", {}, function (res) {
    ...
    function lockPage(){
        var ss = $("#faceimg").html();
        var uname = $("#account").html();
        var nname = $(".userName").html();
        layer.open({
            title : false,
            type : 1,
            content : '<div class="admin-header-lock" id="lock-box">'+
                            '<div class="admin-header-lock-img"><img src="'+ss+'" class="userAvatar"/></div>'+
                            '<div class="admin-header-lock-name"><span id="lockUserName">'+uname+'</span>'+
                            '<span id="lockNickName">'+nname+'</span></div>'+
                            '<div class="input_btn">'+
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".admin-header-lock-input").val() == ''){
            ...
            $.post("/system/indexs", {username:username, password:pwd}, function (res) {
                    if(res.success) {
